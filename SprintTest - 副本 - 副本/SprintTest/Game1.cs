...
public class Game1 : Game
{
    ...
    protected override void Update(GameTime gameTime)
    {
        ...
        // God Mode 提示倒计时
        if (godModeTimer > 0)
            godModeTimer -= gameTime.ElapsedGameTime.TotalSeconds;
        // 血量满血提示倒计时
        if (refillTimer > 0)
            refillTimer -= gameTime.ElapsedGameTime.TotalSeconds;
    ...
    protected override void Draw(GameTime gameTime)
    {
        ...
        if (refillTimer > 0)
        {
            const string text = "FULL HEALTH!";
            Vector2 size = _menuFont.MeasureString(text);
            Vector2 pos = new Vector2(
                (_graphics.PreferredBackBufferWidth - size.X) / 2,
                50);   // 比 God Mode 提示再往下
            _spriteBatch.DrawString(_menuFont, text, pos, Color.Lime);
        }
