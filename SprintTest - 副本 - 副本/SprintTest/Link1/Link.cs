...
namespace sprint0Test.Link1
{
    public class Link
    {
        ...
        {
            if (isInvulnerable) return;        // God Mode 生效时不受伤
            if (!isAttacking && !isUsingItem)
           {
        ...
        {
            // 如果正在攻击，则锁定绘制位置为攻击开始时记录的 attackLockPosition
            Vector2 drawPos = isAttacking ? attackLockPosition : position;
